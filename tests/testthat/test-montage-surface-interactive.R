...
test_that("montage_surface validates its portable display contract", {
  ...
  expect_identical(config$preset, "report")
  expect_null(config$height)
  expect_identical(config$layout, "anatomical")
  expect_identical(config$view, "oblique")
  expect_identical(config$anatomy, "sulcal_depth")
  expect_identical(config$oblique, "auto")
  expect_equal(montage_surface(oblique = c(-120, 20))$oblique, c(-120, 20))
  expect_identical(montage_surface(layout = "split")$layout, "split")
  expect_error(montage_surface(oblique = c(0, 120)), "oblique")
  expect_error(montage_surface(oblique = "front"), "oblique")
  expect_error(montage_surface(view = "sideways"), "should be one of")
  expect_error(montage_surface(anatomy = "thickness"), "should be one of")
...
test_that("render_montage_report builds one synchronized surface scene per analysis", {
  ...
  expect_match(rd$surface$runtime$adapter_version, "^2\\.0\\.0$")
...
test_that("surface report hooks emit a synchronized widget host", {
  ...
  expect_match(document, "3D surface", fixed = TRUE)
  expect_match(document, "setBrainView", fixed = TRUE)
  expect_match(document, "fitInsets", fixed = TRUE)
  expect_match(host, "data-nm-surface-host", fixed = TRUE)
  expect_match(host, "3D surface viewer: ", fixed = TRUE)
  expect_match(host, "data-nm-surface-layout=\"anatomical\"", fixed = TRUE)
  expect_match(host, "data-nm-surface-oblique=\"auto\"", fixed = TRUE)
  expect_match(host, "data-nm-surface-legends", fixed = TRUE)
  expect_match(host, "data-nm-surface-views", fixed = TRUE)
  ...
  expect_match(host, "Surface colour map", fixed = TRUE)
  expect_match(host, "Hide values from", fixed = TRUE)
  expect_match(host, "Show values whose magnitude exceeds", fixed = TRUE)
  expect_match(host, "Colour range high", fixed = TRUE)
  expect_match(host, "Overlay opacity", fixed = TRUE)
  expect_match(host, "Restore report settings", fixed = TRUE)
  expect_match(host, "figure of record", fixed = TRUE)
