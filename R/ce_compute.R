...
# -- Bounded LRU cache --------------------------------------------------------
# A lightweight cache with a max-entry bound.  Works as a drop-in replacement
# for the plain environments used by the public API (series_cache_env).

.new_lru_cache <- function(max_entries = 500L) {
...
# Helpers that work with both plain environments and LRU cache objects
.cache_get <- function(cache, key) {
  if (is.null(cache)) return(NULL)
  if (is.list(cache) && is.function(cache$get)) return(cache$get(key))
...
.cache_set <- function(cache, key, value) {
  ...
  if (is.list(cache) && is.function(cache$set)) {
    cache$set(key, value)
  } else if (is.environment(cache)) {
    assign(key, value, envir = cache)
  }
  invisible(NULL)
}

.cache_exists <- function(cache, key) {
  if (is.null(cache)) return(FALSE)
  if (is.list(cache) && is.function(cache$exists)) return(cache$exists(key))
  if (is.environment(cache)) {
    return(exists(key, envir = cache, inherits = FALSE))
  }
  FALSE
}
...
.cache_clear <- function(cache) {
  if (is.null(cache)) return(invisible(NULL))
  if (is.list(cache) && is.function(cache$clear)) {
    cache$clear()
  } else if (is.environment(cache)) {
...
.extract_series_matrix <- function(data_source,
                                   ...
                                   cache_env = NULL,
                                   cache_key = NULL) {
  if (!is.null(cache_key) && .cache_exists(cache_env, cache_key)) {
    cached <- .cache_get(cache_env, cache_key)
  ...
  if (!is.null(cache_key)) {
    .cache_set(cache_env, cache_key, out)
  }
  out
}
