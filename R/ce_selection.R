.compute_selection_data <- function(selection_engine = c("cluster",
  ...
  # Atlas harmonization is handled by each leaf builder independently
...
.build_parcel_selection_data <- function(data_source,
  ...
  atlas <- aligned$atlas
  if (!is.null(aligned$message)) message(aligned$message)
  if (!is.null(aligned$warning)) warning(aligned$warning, call. = FALSE)
...
.build_sphere_selection_data <- function(data_source,
  ...
  if (is.null(centers)) {
    ...
    centers <- if (identical(sphere_units, "mm")) {
      neuroim2::grid_to_coord(vol_space, peak_grid)
    } else {
      peak_grid
    }
  }

  # Bounding-box pre-filter: only generate grid within reach of any sphere
  if (identical(sphere_units, "mm")) {
    center_grids <- neuroim2::coord_to_grid(vol_space, centers)
    vox_spacing <- abs(neuroim2::spacing(vol_space))
    pad <- as.integer(ceiling(sphere_radius / min(vox_spacing))) + 1L
  } else {
    center_grids <- centers
    pad <- as.integer(ceiling(sphere_radius)) + 1L
  }
  bbox_lo <- pmax(1L, as.integer(floor(apply(center_grids, 2, min))) - pad)
  bbox_hi <- pmin(dims3, as.integer(ceiling(apply(center_grids, 2, max))) + pad)
  bbox_grid <- as.matrix(expand.grid(
    x = seq.int(bbox_lo[1], bbox_hi[1]),
    y = seq.int(bbox_lo[2], bbox_hi[2]),
    z = seq.int(bbox_lo[3], bbox_hi[3])
  ))
  bbox_linear <- .grid_to_linear_index(bbox_grid, dims3)
  coord_bbox <- if (identical(sphere_units, "mm")) {
    neuroim2::grid_to_coord(vol_space, bbox_grid)
  } else {
    bbox_grid
  }
  ...
  if (identical(sphere_combine, "union")) {
    keep <- rep(FALSE, nrow(coord_bbox))
    for (i in seq_len(nrow(centers))) {
      d2 <- rowSums((coord_bbox - matrix(centers[i, ], nrow(coord_bbox), 3,
                                         byrow = TRUE))^2)
      keep <- keep | (d2 <= sphere_radius^2)
    }
    idx <- which(keep)
    if (length(idx) > 0) {
      vox <- bbox_grid[idx, , drop = FALSE]
      svals <- as.numeric(stat_arr[bbox_linear[idx]])
      peak <- svals[which.max(abs(svals))]
