...
.cluster_explorer_server <- function(input, output, session,
  ...
  cache_state <- shiny::reactiveValues(
    series_cache = .new_lru_cache(max_entries = 500L),
    ts_cache = .new_lru_cache(max_entries = 500L)
  )
  ...
  shiny::observeEvent(computed(), {
    dat <- computed()
    cache_state$series_cache <- .new_lru_cache(max_entries = 500L)
    cache_state$ts_cache <- .new_lru_cache(max_entries = 500L)

    if (nrow(dat$cluster_ts) > 0) {
      split_ts <- split(dat$cluster_ts, dat$cluster_ts$cluster_id)
      for (cid in names(split_ts)) {
        .cache_set(cache_state$ts_cache, cid, split_ts[[cid]])
      }
    }
  ...
  output$cluster_table <- DT::renderDT({
    ...
    if (nrow(tbl) == 0) {
      empty_tbl <- tibble::tibble(
        Message = "No clusters found. Try lowering the threshold or min cluster size, or changing the tail setting."
      )
      return(DT::datatable(
        empty_tbl,
        rownames = FALSE,
      ...
      g <- ggiraph::girafe_options(
        ...
        ggiraph::opts_tooltip(
          css = paste(
            ...
            "padding:7px 9px;font-size:11px;font-family:system-ui,-apple-system,sans-serif;"
          )
        )
      )
    }
  ...
  output$signal_plot <- ggiraph::renderGirafe({
    ...
    ggiraph::girafe_options(
      g,
      ggiraph::opts_hover(css = "stroke:#111;stroke-width:0.5;r:4;"),
      ggiraph::opts_tooltip(
        css = paste(
          "background:#111827;color:#f9fafb;border:none;border-radius:8px;",
          "padding:7px 9px;font-size:11px;font-family:system-ui,-apple-system,sans-serif;"
...
#'   prefetch.
#' @param palette Continuous palette passed to \code{\link[neuroatlas]{plot_brain}()}.
            ...
            shiny::column(
              ...
              shiny::div(
                ...
                shiny::div(
                  ...
                  ggiraph::girafeOutput("signal_plot", height = "430px")
                )
              )
            )
          ),
